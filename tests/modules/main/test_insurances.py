...
def test_insurance_pension(default_manager):
    ...
    assert default_manager.get_module("pension").expected_income == pytest.approx(
        13860.0)  # TODO test with real numbers
    default_manager.next_year()
    assert default_manager.get_module("pension").entgeltpunkte == pytest.approx(10.5)
    assert default_manager.df_row["expenses"] == pytest.approx(1808.8965)
    # getting money
    default_manager.profile.retired = True
    default_manager.next_year()
    assert default_manager.df_row["income"] == 4158
...
def test_insurance_nursing_care(default_manager):
    ...
    default_manager.next_year()
    assert default_manager.df_row["expenses"] == pytest.approx(40000 * 0.0165)
    default_manager.profile.retired = True
    default_manager.next_year()
    assert default_manager.df_row["expenses"] == pytest.approx(40000 * 0.0165 * 2)
...
def test_unemployment(default_manager):
    ...
    assert default_manager.df_row["expenses"] == pytest.approx(30000 * 1.02 * 0.024 * 0.5)
